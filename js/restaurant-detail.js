...
function renderEquipements() {
    ...
    customEquipementsList.querySelectorAll(".custom-equip-rename-btn").forEach(function (btn) {
        btn.addEventListener("click", function () {
            ...
            var oldKey = "restaurant_" + currentRestaurant.numero + "_customeq_" + ancienNom;
            var newKey = "restaurant_" + currentRestaurant.numero + "_customeq_" + nouveauNom;
...
function renderEclairages() {
    ...
    eclairages.forEach(function (item, index) {
        ...
        row.innerHTML =
            ...
            '<div class="row-photo-group row-photo-fullwidth">' + PhotoManager.createMultiPhotoWidget("restaurant_" + currentRestaurant.numero + "_eclairage_" + index) + '</div>';
