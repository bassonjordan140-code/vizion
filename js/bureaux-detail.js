...
function renderEquipements() {
    ...
    customEquipementsList.querySelectorAll(".custom-equip-rename-btn").forEach(function (btn) {
        btn.addEventListener("click", function () {
            ...
            var oldKey = "bureaux_" + currentBureaux.numero + "_customeq_" + ancienNom;
            var newKey = "bureaux_" + currentBureaux.numero + "_customeq_" + nouveauNom;
...
function renderEclairages() {
    ...
    eclairages.forEach(function (item, index) {
        ...
        row.innerHTML =
            ...
            '<div class="row-photo-group row-photo-fullwidth">' + PhotoManager.createMultiPhotoWidget("bureaux_" + currentBureaux.numero + "_eclairage_" + index) + '</div>';
